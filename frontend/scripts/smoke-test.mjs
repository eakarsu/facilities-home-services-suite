const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:3820';
...
const adminCookie = await login('admin@facilities-home.local', 'admin123');
const managerCookie = await login('manager@facilities-home.local', 'manager123');
const analystCookie = await login('analyst@facilities-home.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/work-orders', analystCookie);
await expectJson('/api/documents', analystCookie);
await expectStatus('/api/documents/upload', analystCookie, 405);

const records = await expectJson('/api/entities/work-orders', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/work-orders/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/work-orders/approve`, {
  method: 'POST',
  headers: {
